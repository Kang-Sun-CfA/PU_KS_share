function output = F_subset_CrIS(inp)
% Matlab function to subset CrIS data from Shephard and Cady-Pereira.
% Written by Kang Sun on 2018/10/15
% updated on 2018/11/01 to include total column error
...
varname = {'DOF','Day_Night_Flag','LandFraction','Latitude','Longitude'...
    ...
    'total_covariance_error','noise_error_covariance','pressure','xa','avg_kernel'};
latall = single([]);
lonall = latall;
ifovall = latall;
colnh3all = latall;
...
dofall = latall;
for iday = 1:nday
    day_dir = [L2dir,'CrIS_NH3_global_v1_3_n170_0_n050_0_p015_0_p090_0_',...
        num2str(year_array(iday)),'_',num2str(month_array(iday),'%02d'),...
        '/',num2str(year_array(iday)),'/',...
