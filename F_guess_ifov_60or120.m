function outp = F_guess_ifov_60or120(inp)

% rewriting F_guess_ifov.m and F_guess_ifov_incomplete.m, because the same
% day (2010/03/03) may have both 120 fov and 60 ifov!!!
...
ref_anglen120 = [29,29,30,30,27,27,28,28,25,25,26,26,23,23,24,24,21,21,22,22,19,19,20,20,17,17,18,18,15,15,16,16,13,13,14,14,11,11,12,12,9,9,10,10,7,7,8,8,5,5,6,6,3,3,4,4,1,1,2,2,2,2,1,1,4,4,3,3,6,6,5,5,8,8,7,7,10,10,9,9,12,12,11,11,14,14,13,13,16,16,15,15,18,18,17,17,20,20,19,19,22,22,21,21,24,24,23,23,26,26,25,25,28,28,27,27,30,30,29,29];
ref_anglen60 = ref_anglen120(1:2:end);
pattern_str120 = [1 1 2 2 2 2 1 1];
pattern_str60 = [1 2 2 1];
for iorbit = 1:length(int_orbit_cell)
    
    orbit_anglen = anglen(int_orbit_cell{iorbit});
    orbit_anglen = orbit_anglen(:)';
    
    if ~isempty(strfind(orbit_anglen,ref_anglen120))
        true_120_false_60 = true;
        center_ind = strfind(orbit_anglen,pattern_str120)-1+int_orbit_cell{iorbit}(1);
        ref_anglen = ref_anglen120;
        ref_anglen_pm = ref_anglen;
        ref_anglen_pm(61:end) = -ref_anglen_pm(61:end);
        pattern_str = pattern_str120;
    elseif ~isempty(strfind(orbit_anglen,ref_anglen60))
        true_120_false_60 = false;
        center_ind = strfind(orbit_anglen,pattern_str60)-1+int_orbit_cell{iorbit}(1);
        ref_anglen = ref_anglen60;
        ref_anglen_pm = ref_anglen;
        ref_anglen_pm(31:end) = -ref_anglen_pm(31:end);
        pattern_str = pattern_str60;
    else
        disp(['Orbit #',num2str(iorbit),' is neither 120 fov nor 60 fov!'])
        continue
    end
    ...
    if true_120_false_60
        ...
        disp(['Orbit # ',num2str(iorbit),' (120 fov) finished...'])
    else
        for ifull_scan = 1:length(full_scan_ind)
            full_scan_int = full_scan_ind(ifull_scan):full_scan_ind(ifull_scan)+59;
            if mean(UTC(full_scan_int(30:31)))-UTC(full_scan_int(1)) < abs(ttbdr(1)) ...
                    && UTC(full_scan_int(end))-mean(UTC(full_scan_int(30:31))) < abs(ttbdr(end))
                ifov(full_scan_int) = 1:2:120;
                
                [mintdiff,I] = min(abs(mean(UTC(full_scan_int(30:31)))-center_time1));
                if mintdiff > 0.1/86400
                    error('time matching went wrong')
                end
                center_time2(I) = nan;
        ...
        for iscan = 1:length(center_time2)
            scan_center_time = center_time2(iscan);
            [binc,C] = histc(UTC,ttbdr+scan_center_time);
            full_position = find(binc == 2);
            full_fov = [full_position*4-3;full_position*4-1];
            full_fov = sort(full_fov);
            full_int = ismember(C,full_position);
            ifov(full_int) = full_fov;
            for iposition = position_array(binc(1:30)' > 0 & binc(1:30)' < 2)
                int_bad_position = find(C == iposition);
                local_anglen = anglen(int_bad_position);
                if iposition > 15
                    local_anglen = -local_anglen;
                ...
                if isempty(guess_ind)
                    ...
                    ifov(int_bad_position) = guess_ind(1)*2-1;
                end
                
            end
        end
        disp(['Orbit # ',num2str(iorbit),' (60 fov) finished...'])
    end
