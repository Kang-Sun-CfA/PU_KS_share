function outp = F_guess_ifov_incomplete(inp)

% modified from F_guess_ifov.m to handle incomplete iasi data with only
% have of ifov

% by Kang Sun on 2017/12/28
...
ref_anglen = ref_anglen(1:2:end);
if isempty(strfind(anglen(:)',ref_anglen))
    error('60 fovs are not complete!')
end
...
%         plot(lon_up,lat_up,'o',lon_lo,lat_lo,'o',lon_ct,lat_ct,'o')
